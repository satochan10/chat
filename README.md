...
- 初回アクセス時に「名前」と「4桁のパスワード」を入力してログイン（くまの絵文字とピンク×パープルのグラデーションを使った、可愛らしいデザイン）
...
- ログイン後のホーム画面で、自分のアイコン（絵文字）と名前、「友達」一覧、「誰とチャットするか」の選択を1画面にまとめて表示
  - 自分のアイコンボタンを押すと固定の絵文字リスト（🐻🐰🐶など16種）が開き、選んだ絵文字がFirestoreの `users` ドキュメントに保存される（他人の名前が分かれば変更できてしまう簡易的な仕組み。身内利用を想定）
  - 「友達」欄・申請欄には、それぞれの相手のアイコンも表示される
  - 「友達」だけが一覧表示され、その中から「誰とチャットするか」を選択（LINEの友達追加に近い、承認制）
  - 「友達」一覧の各相手について、その相手から自分が最後にそのチャットを開いた時刻より新しいメッセージが届いている場合、名前の右にピンクの丸い未読マークを表示する（既読状態はブラウザのlocalStorageに保存。ブラウザ・端末をまたぐと反映されない）
  - ホーム画面右上の「⟳」でページ全体をハードリロード（キャッシュを避けて最新版を取得）する
  - 「友達を追加」フォームに登録済みの名前を入力すると申請（Firestoreの `friendships` コレクションに `pending` の状態で保存）
  - 相手から届いた申請は「届いている申請」欄に表示され、承認（`accepted` に更新）または拒否（ドキュメント削除）できる
  - 自分が送った申請は「申請中」欄に表示される（相手が承認するまでチャットは開始できない）
  - `accepted` になった相手だけが「友達」欄に表示され、チャットを開始できる（Firestoreセキュリティルールでもメッセージ送信時に友達関係を検証）
- 選択した相手との1対1チャット（Firestoreの `messages` コレクションを、2人の名前から生成した `conversationId` で絞り込んで表示）
  - 自分のメッセージは右・ピンク×パープルのグラデーション吹き出し、相手のメッセージは左・淡いピンクの吹き出し（送信者名付き）で表示。ログイン画面と統一した可愛らしいデザイン（丸ゴシック体・グラデーション背景）
  - 各メッセージの横に送信日時（時:分）を表示
  - 送信は削除・編集不可（Firestoreセキュリティルールで制限）
  - メッセージ入力欄が空のときは送信ボタンが非活性になる
  - 入力欄横の「😀」ボタンでスタンプ（👍❤️🎉の3種、絵文字）を選んで送信できる（テキストなしで即送信、吹き出し背景なしの大きな絵文字として表示。Firestoreセキュリティルールで送信できるスタンプの種類を固定リストに制限）
  - チャット画面上部に相手のアイコン・名前を表示
  - チャット画面左上の「←」でホーム画面に戻れる
  - チャット画面右上の「⟳」でページ全体をハードリロード（キャッシュを避けて最新版を取得）する
  - チャットを開くとブラウザの通知許可をリクエストし、許可済みならチャットを開いている間（タブが非アクティブな時）に相手からの新着メッセージをブラウザ通知（Notification API）で知らせる。タブを閉じる・チャットを離れる（ホームに戻る）と通知は届かなくなる（Service Workerを使わない静的サイトのため、バックグラウンドプッシュ通知には未対応）

## ファイル構成

- `index.html` — ページ構造（ログイン画面・ホーム画面（アイコン設定・友達一覧）・チャットヘッダー・メッセージ一覧・入力フォーム）
...
- 友達の一覧・申請一覧には検索や「最近やり取りした相手」順の並び替えはない
- 友達承認の仕組みは「名前」単位であり、Firebase Authのユーザーとは紐付いていない（4桁PINと同様、名前さえ分かれば別人がなりすませる可能性がある。身内利用を前提とした簡易的な制御）
- `conversationId`（等価条件）と `createdAt`（並び替え）を組み合わせたクエリを使っているため、初回実行時にFirestoreコンソールから複合インデックス作成を促されることがある（表示されるリンクから作成すればよい）
- 通知はチャット画面を開いている間（ページ・タブが開いた状態）のみ機能し、タブやアプリを閉じている間の通知は届かない（Service Worker／プッシュ通知は未実装）
